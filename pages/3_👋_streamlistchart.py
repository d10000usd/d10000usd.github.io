...
class StockDataSearch():
    ...
    def get_stock_rank_list(self,compare,df,dicts):
        ...
        if compare =='상승':
            ...
            df.to_json('data/json/stocklist_whitelist.json',orient='records',force_ascii=False)
        elif compare == '하락':
            df=df[((df['ChagesRatio'] < dicts['days_rate'] ))]
            df.to_json('data/json/stocklist_blacklist.json',orient='records',force_ascii=False)
    ...
    def 주식이름으로조회종목코드(self,ticker_):
        #문자를 포함하는 행을 찾아서 출력
        df = pd.read_csv("data/csv/stockTicker.csv")
...
class PlotlyChartDraw():
    ...
    def get_candlestick_plot(self,df,dicts):
        ...
        savePathes_    =f"data/img/{ud}_{ticker}.png"
